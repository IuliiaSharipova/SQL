...
--	1. Add column 'country' to dept table in mywork database
    ALTER TABLE mywork.dept ADD COLUMN country varchar(100);
...
SELECT first_name, last_name FROM library_simple.author WHERE last_name='Swanson';
--	2. How many pages are in 'Men Without Fear' book?
SELECT name, page_num FROM library_simple.book WHERE name LIKE '%Men%Without%Fear%';
--	3. Show all book categories that start with letter 'W'
SELECT DISTINCT(name) FROM library_simple.category WHERE name LIKE 'W%';
-- ---------------------------------------------------
